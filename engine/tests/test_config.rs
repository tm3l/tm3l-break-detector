...
use openapiv3::{OpenAPI, Paths, PathItem, ReferenceOr, Operation, Responses};

fn make_api_with_get(path: &str) -> OpenAPI {
    let mut api = OpenAPI::default();
    let mut paths = indexmap::IndexMap::new();
    paths.insert(path.to_string(), ReferenceOr::Item(PathItem {
        get: Some(Operation {
    ...
    api.paths = Paths { paths, extensions: indexmap::IndexMap::new() };
    api
}
...
fn test_ignore_removed_endpoint() {
    let base = make_api_with_get("/users");
    let target = OpenAPI::default();
    let mut config = TM3LConfig::default();
    config.ignore_rules.push("removed_endpoint".to_string());
    let res = compare_specs(&base, &target, &config);
...
fn test_allow_type_coercion_suppresses_type_change() {
    ...
    assert_eq!(strict_res.summary.breaking, 1, "Strict mode must detect type change");

    let mut lenient_config = TM3LConfig::default();
    lenient_config.allow_type_coercion = true;
    let lenient_res = compare_specs(&base, &target, &lenient_config);
    assert_eq!(lenient_res.summary.breaking, 0, "allow_type_coercion must suppress type change");
}
